...
import org.idea.liyu.live.framework.redis.starter.key.BankProviderCacheKeyBuilder;
import org.liyu.live.bank.constants.TradeTypeEnum;
import org.liyu.live.bank.dto.AccountTradeReqDTO;
import org.liyu.live.bank.dto.AccountTradeRespDTO;
import org.liyu.live.bank.provider.dao.mapper.ILiyuCurrencyAccountMapper;
 ...
 */
@Service
public class LiyuCurrencyAccountServiceImpl implements ILiyuCurrencyAccountService {

    @Resource
    private ILiyuCurrencyAccountMapper liyuCurrencyAccountMapper;
    @Resource
    private RedisTemplate<String, Object> redisTemplate;
    @Resource
    private BankProviderCacheKeyBuilder cacheKeyBuilder;
    @Resource
    private ILiyuCurrencyTradeService currencyTradeService;
    ...
    public boolean insertOne(long userId) {
        try {
            LiyuCurrencyAccountPO accountPO = new LiyuCurrencyAccountPO();
            accountPO.setUserId(userId);
            liyuCurrencyAccountMapper.insert(accountPO);
            return true;
        } catch (Exception e) {
        }
        return false;
    ...
    public Integer getBalance(long userId) {
        ...
        Integer currentBalance = liyuCurrencyAccountMapper.queryBalance(userId);
    ...
    public void consumeIncrDBHandler(long userId, int num) {
        //更新db，插入db
        liyuCurrencyAccountMapper.incr(userId, num);
        //流水记录
    ...
    public void consumeDecrDBHandler(long userId, int num) {
        //更新db，插入db
        liyuCurrencyAccountMapper.decr(userId, num);
        //流水记录
...
//        LiyuCurrencyAccountDTO accountDTO = this.getByUserId(userId);
//        if (accountDTO == null) {
