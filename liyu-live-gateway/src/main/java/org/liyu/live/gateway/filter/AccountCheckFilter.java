package org.liyu.live.gateway.filter;
...
import org.liyu.live.gateway.properties.GatewayApplicationProperties;
...
public class AccountCheckFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        headers.add(HttpHeaders.ACCESS_CONTROL_ALLOW_ORIGIN, "http://192.168.222.1:5500");
        ...
        List<HttpCookie> httpCookieList = request.getCookies().get("lytk");
        if (CollectionUtils.isEmpty(httpCookieList)) {
            LOGGER.error("请求没有检索到lytk的cookie，被拦截");
            return Mono.empty();
        }
        String liyuTokenCookieValue = httpCookieList.get(0).getValue();
        if (StringUtils.isEmpty(liyuTokenCookieValue) || StringUtils.isEmpty(liyuTokenCookieValue.trim())) {
            LOGGER.error("请求的cookie中的lytk是空，被拦截");
            return Mono.empty();
        }
        //token获取到之后，调用rpc判断token是否合法，如果合法则吧token换取到的userId传递给到下游
        Long userId = accountTokenRPC.getUserIdByToken(liyuTokenCookieValue);
